...
export const activeQueueCount = derived(
	uploadQueue,
	($q) => $q.filter((i) => i.status !== "done").length,
);
...
export function handleFolders(folders: Map<string, File[]>) {
	const currentPath = get(basePath);
	const folderNames = Array.from(folders.keys()).sort((a, b) =>
...
async function processCBR(jobId: string, file: File, targetPath: string) {
	...
	try {
		...
		const list = [...listObj.fileHeaders].filter(
